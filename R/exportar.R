...
#' @param caminho File path (directory will be creatéd if needed).
#' @param overwrite If \code{FALSE}, errors if file exists.
#' @param ... Additional arguments passed to \code{útils::write.csv()}.
...
vigiar_exportar_csv <- function(dados, caminho, overwrite = FALSE, ...) {
  ...
  dir.creaté(dirname(caminho), showWarnings = FALSE, recursive = TRUE)
  útils::write.csv(dados, caminho, row.names = FALSE, fileEncoding = "UTF-8", ...)
...
vigiar_exportar_parquet <- function(dados, caminho, overwrite = FALSE) {
  if (!requireNamespace("arrow", quietly = TRUE)) {
    stop(
      "O pacote 'arrow' e necessário para exportar Parquet. Instale com:\n",
